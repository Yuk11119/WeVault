...
export const createApp = async (deps: { config: Config; db: Database; mailer: Mailer; objectStore: ObjectStore }): Promise<FastifyInstance> => {
  ...
  // Nginx is the sole public-facing hop and rewrites X-Forwarded-For to the
  // peer address. Trusting only loopback lets rate limiting use that address
  // without accepting a client-supplied forwarding chain.
  const app = Fastify({
    trustProxy: "127.0.0.1",
    logger: { redact: ["req.headers.authorization", "req.body.password", "req.body.refreshToken", "req.body.code", "res.body.accessToken", "res.body.refreshToken"] }
  });
  ...
  app.post("/v1/objects/:authorizationId/complete", async request => { const identity = auth(config, request); const params = z.object({ authorizationId: uuid }).parse(request.params); const authz = await db.query("SELECT * FROM object_authorizations WHERE id=$1 AND user_id=$2 AND operation='UPLOAD' AND consumed_at IS NULL AND expires_at>now()", [params.authorizationId, identity.userID]); if (!authz.rowCount) throw errors.notFound(); const row = authz.rows[0]!; await ownDevice(db, identity.userID, row.device_id); const head = await objectStore.head(row.object_key); if (head.sizeBytes !== Number(row.expected_size_bytes) || head.sha256 !== row.expected_sha256) throw errors.cloud("Remote object metadata does not match the authorized object"); const objectID = randomUUID(); const saved = await transaction(db, async client => { const result = await client.query("INSERT INTO archive_objects (id,user_id,device_id,sha256,size_bytes,object_key,status,verified_at) VALUES ($1,$2,$3,$4,$5,$6,'VERIFIED',now()) ON CONFLICT (user_id,device_id,sha256) DO UPDATE SET status='VERIFIED',verified_at=now() RETURNING id,sha256,size_bytes,verified_at", [objectID, identity.userID, row.device_id, row.expected_sha256, row.expected_size_bytes, row.object_key]); await client.query("UPDATE object_authorizations SET consumed_at=now(),archive_object_id=$1 WHERE id=$2", [result.rows[0]!.id, row.id]); return result.rows[0]!; }); await audit(db, "OBJECT_VERIFIED", { sha256: row.expected_sha256 }, identity.userID, row.device_id); return { status: "VERIFIED", objectId: saved.id, sha256: saved.sha256, sizeBytes: Number(saved.size_bytes), verifiedAt: saved.verified_at }; });
  app.post("/v1/objects/:objectId/download-authorizations", async request => { const identity = auth(config, request); const params = z.object({ objectId: uuid }).parse(request.params); const body = z.object({ deviceId: uuid }).parse(request.body); await ownDevice(db, identity.userID, body.deviceId); const object = await db.query("SELECT * FROM archive_objects WHERE id=$1 AND user_id=$2 AND (device_id=$3 OR device_id IN (SELECT a.source_device_id FROM device_archive_access a JOIN devices source ON source.id=a.source_device_id WHERE a.device_id=$3 AND source.user_id=$2 AND source.revoked_at IS NULL)) AND status='VERIFIED'", [params.objectId, identity.userID, body.deviceId]); if (!object.rowCount) throw errors.notFound(); const row = object.rows[0]!; const authorizationID = randomUUID(); await db.query("INSERT INTO object_authorizations (id,user_id,device_id,archive_object_id,operation,object_key,expected_sha256,expected_size_bytes,expires_at) VALUES ($1,$2,$3,$4,'DOWNLOAD',$5,$6,$7,now()+interval '15 minutes')", [authorizationID, identity.userID, body.deviceId, row.id, row.object_key, row.sha256, row.size_bytes]); return { authorizationId: authorizationID, credentials: await objectStore.issue("download", row.object_key) }; });
  app.get("/v1/objects", async request => { const identity = auth(config, request); const query = z.object({ deviceId: uuid, sha256 }).parse(request.query); await ownDevice(db, identity.userID, query.deviceId); const object = await db.query("SELECT id,sha256,size_bytes,object_key,verified_at FROM archive_objects WHERE user_id=$1 AND (device_id=$2 OR device_id IN (SELECT a.source_device_id FROM device_archive_access a JOIN devices source ON source.id=a.source_device_id WHERE a.device_id=$2 AND source.user_id=$1 AND source.revoked_at IS NULL)) AND sha256=$3 AND status='VERIFIED'", [identity.userID, query.deviceId, query.sha256]); return { objects: object.rows.map(row => ({ objectId: row.id, sha256: row.sha256, sizeBytes: Number(row.size_bytes), verifiedAt: row.verified_at })) }; });
  return app;
};
