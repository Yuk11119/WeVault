...
| POST | `/objects/upload-authorizations` | Return an exact-object, 15-minute upload STS credential |
| POST | `/objects/:authorizationId/complete` | Server HEAD/metadata verification and object-index write |
| POST | `/objects/:objectId/download-authorizations` | Return an exact-object, 15-minute download STS credential |
...
2. Point `api.wevault.online` to the ECS, install the dedicated Nginx vhost, obtain a Let’s Encrypt certificate, and confirm `curl https://api.wevault.online/healthz` returns `{"status":"ok"}`. Do not expose 31061 or 5433.
...
8. Install the Nginx site, run `nginx -t`, request the certificate with `certbot --nginx -d <api-domain>`, reload Nginx, then confirm `curl https://<api-domain>/healthz` returns `{"status":"ok"}`.
