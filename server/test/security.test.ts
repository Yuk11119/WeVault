...
import { cosPolicyFor, objectKeyFor, policyFor } from "../src/oss.ts";
...
test("COS session policy is restricted to one object and operation", () => {
  const key = objectKeyFor("user-1", "device-1", "b".repeat(64));
  const policy = cosPolicyFor("qcs::cos:ap-shanghai:uid/1:bucket/wevault-1", key, "download");
  assert.match(policy, /name\/cos:GetObject/); assert.doesNotMatch(policy, /PutObject/); assert.ok(policy.includes(key));
});
