...
- 版本：0.7.1 / build 8，macOS 14+，arm64 + x86_64。
- 文件：`WeVault-0.7.1-8-macos-universal.zip`。
- SHA-256：`07a7a11a32338e6bbc8f296450073df288f5b7a317b3a9b94f4116d091f4c438`。
...
- 站点目录：`/var/www/wevault-site/releases/0.7.1-8`，`/var/www/wevault-site/current` 指向该目录。
- HTTPS：独立 Let's Encrypt 证书，签发时到期日 2026-12-08；`certbot.timer` 已启用。续期使用 `/var/www/certbot`，站点证书续期后 hook 检查并 reload Nginx。
- 本地静态产物：`.build/website-0.7.1-8-r2`。只部署该目录内容，源码、私钥、应用索引和测试夹具不上传。
...
```sh
WEVAULT_VERSION=0.7.2 WEVAULT_BUILD=9 \
...
python3 Website/build.py --release .build/public-beta/0.7.2-9 --output .build/website-0.7.2-9
